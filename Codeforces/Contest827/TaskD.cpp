...
int main(int argc, char const *argv[])
{
	...
	for (iii e : E) {
		...
		if (dsu.merge(u, v)) {
			adj[u].push_back({v, e.first.first});
			adj[v].push_back({u, e.first.first});
			inTree[i] = 1;
		}
		i++;
	}
	...
	for (int i = 0; i < m; ++i) if (!inTree[i]) {
		ans[E[i].first.second] = lca(E[i].second.first, E[i].second.second, E[i].first.first) - 1;
	}
	...
	for (iii e : E) {
		...
		ans[e.first.second] = mi[e.second.second][0] - 1;
		if (ans[e.first.second] == (int)1e9 + 6) ans[e.first.second] = -1;
		i++;
	}
	for (int i = 1; i <= m; ++i)
		cout << ans[i] << ' ';
